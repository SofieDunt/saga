...
public class StatusUpdateCreator {
  ...
  public static StatusUpdate importSimple(Scanner sc) throws IllegalArgumentException {
    ...
    String illegalFormatMessage = "Illegal format - not a valid status update";
    String stringType = IOUtils.tryNext(sc, illegalFormatMessage);
    StatusUpdateTypes type;
    try {
      type = StatusUpdateTypes.valueOf(stringType);
    } catch (IllegalArgumentException e) {
      throw new IllegalArgumentException(illegalFormatMessage);
    }

    return StatusUpdateCreator.createSimple(type, IOUtils.tryNextInt(sc, illegalFormatMessage));
  }
